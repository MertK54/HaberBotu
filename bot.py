...
def anlik_piyasa_verisi():
    try:
        ...
        return (f"📊 *FİNANSAL DURUM RAPORU* ({zaman})\n"
                ...
                f"🥈 *Gümüş Ons:* {fiyatlar['Gümüş (Ons)']:.2f} $\n"
    ...
    if bulunan_haberler:
        ...
        try:
            prompt = (f"Analyze the following Turkish financial news. Provide a structured summary in Turkish. "
                      f"Highlight key points using bolding, categorize points using emojis, "
                      f"and explain the likely market impact. News: {haber_metni}")
...
async def test_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏳ Gümüş dahil tüm veriler çekiliyor...")
...
if __name__ == '__main__':
    ...
    print("🚀 Bot Aktif! Gümüş takibi başladı.")
    app.run_polling()